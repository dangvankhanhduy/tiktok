...
class Tiktok:
    def __init__(self, video_url, threads) -> None:
    ...
    def _banner(self) -> None:
        views, likes, comments, shares = self._videoInfo()
        Write.Print(f"""
\t\t                          d8, d8b                        d8b                                      
\t\t                   d8P   `8P  ?88          d8P           ?88                                      
...
\t╔════════════════════════════════════════════════════════════════════════════════════════════════════╗
\t\t\t\t\t\tVideo ID: {self.video_url.rsplit("/", 1)[-1]}
\t\t\t\t\t\tVideo Owner: @{self.video_url.split('/')[3][1:]}
...
""", Colors.blue_to_purple, interval=0.0000)
    ...
    def _videoInfo(self) -> tuple[int, int, int, int]:
        ...
        r = Session().get(f'https://tiktok.livecounts.io/video/stats/{self.video_url.rsplit("/", 1)[-1]}', headers=headers)
...
def view_booster(video_url, threads):
    try:
        tiktok = Tiktok(video_url, threads)
        tiktok._sendCaptcha()
        tiktok._sendViews()
    except:
        pass
...
if __name__ == "__main__":
    # Nhập video_url và threads từ bàn phím
    video_url = input("Nhap URL video TikTok: ")
    threads = int(input("Nhap so luong (threads): "))

    i = Tiktok(video_url, threads)
    i._banner()
    
    try:
        while True:
            while threading.active_count() - 1 < threads:
                threading.Thread(target=view_booster, args=(video_url, threads)).start()
            time.sleep(1)
